      ...
      <div className="w-full max-w-md rounded-xl bg-white shadow-xl">
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">Bắt đầu</label>
              <input
                type="datetime-local"
                value={startAt}
            ...
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">Kết thúc</label>
              <input
                type="datetime-local"
                value={endAt}
            ...
            {courses.map((c) => (
              <tr key={c.id} className="hover:bg-slate-50/70">
                ...
                <td className="px-4 py-2 text-xs text-slate-500">
                  <div>
                    {c.start_at ? new Date(c.start_at).toLocaleString("vi-VN") : "—"}
                  </div>
                  <div>
                    {c.end_at ? new Date(c.end_at).toLocaleString("vi-VN") : "—"}
                  </div>
                </td>
                <td className="px-4 py-2">
                  <button
